...
Presets are `unilateral_10`, `bilateral_10`, and `custom_path`. There is intentionally no free-trade transition preset. The solution horizon is fixed at 80 transition periods. A custom request replaces the target/profile fields with `tau21Path` and `tau12Path`. Each path may contain 1 to 80 positive gross tariffs; its final supplied value is carried forward through period 80.
